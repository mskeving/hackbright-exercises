...
def make_new_project(*args):
    ...
    new_args = text.split('" ')
    try:
        int(new_args[len(new_args)-1])
    except ValueError:
        print "You must enter a numerical maximum grade."
 #       main()
    title = new_args[0].strip('"')
    description = new_args[1].strip('"')
    max_grade = int(new_args[len(new_args)-1])
    existing_query = """ SELECT title FROM Projects WHERE title = ?"""
 ...
 #       main()
    ...
    CONN.commit()
    print "Successfully added a project: %s %s %d" % (title,description,max_grade)
